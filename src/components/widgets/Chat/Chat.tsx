...
export const Chat: React.FC<ChatPageProps> = (props) => {
        ...
        (acc, [key, value]) => ({
          ...
          ...{
            [key]: {
              ...
              updatedDate: _utils.functions.formatDate(value.updatedDate),
            },
          },
        }),
        {}
  ...
  return (
    <Container className='main-container'>
      <Flex>
        <Sidebar className='left-side' chatId={props.id} />
                ...
                <Wrapper>
                  ...
                  {messages.length === 1 && !props.id && (
                    <ButtonGroup className='wrap start-message gap-15'>
                      {START_PROMPTS &&
                        START_PROMPTS.map((item, key) => (
                          <Button onClick={() => handlePrompt(item)} key={key}>
                            {item}
                          </Button>
                        ))}
                    </ButtonGroup>
